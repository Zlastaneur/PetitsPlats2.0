...
ingredientBtn.addEventListener("click", () => {
    isIngredientsOpen = toggleDropdownButton(
        isIngredientsOpen,
        ingredientDropdown,
        document.querySelector(".chevron_ingredient"),
    )
})
applianceBtn.addEventListener("click", () => {
    isAppliancesOpen = toggleDropdownButton(
        isAppliancesOpen,
        applianceDropdown,
        document.querySelector(".chevron_appliance"),
    )
})
utensilBtn.addEventListener("click", () => {
    isUtensilsOpen = toggleDropdownButton(
        isUtensilsOpen,
        utensilDropdown,
        document.querySelector(".chevron_utensil"),
    )
})

ingredientSearchInput.addEventListener("input", () =>
    handleFilterInput(ingredientSearchInput, ingredientDropdown),
)
applianceSearchInput.addEventListener("input", () =>
    handleFilterInput(applianceSearchInput, applianceDropdown),
)
utensilSearchInput.addEventListener("input", () =>
    handleFilterInput(utensilSearchInput, utensilDropdown),
)
...
function createIngredientsList() {
    const ingredients = recipesData.flatMap((recette) =>
        recette.ingredients.map((ing) => ing.ingredient.toLowerCase()),
    )
    handleFiltersListCreation(
        ".ingredient_list",
        ingredientDropdown,
        ingredients,
        "ingredients",
...
function createAppliancesList() {
    const appliances = recipesData.flatMap((recette) =>
...
function createUtensilsList() {
    const utensils = recipesData.flatMap((recette) =>
        recette.ustensils.map((utensil) =>
            utensil.replace(/[^a-zA-ZÀ-ÿ\s]/g, "").toLowerCase(),
        ),
    )
    handleFiltersListCreation(
        ".utensil_list",
...
function handleFiltersListCreation(
            ...
            recipesMatchingData = data.filter((ingredient) =>
                displayedRecipes.some((recipe) =>
                    recipe.ingredients.some(
                        (ing) => ing.ingredient.toLowerCase() === ingredient,
                    ),
                ),
            )
            break
            ...
            recipesMatchingData = data.filter((utensil) =>
                displayedRecipes.some((recipe) =>
                    recipe.ustensils.some(
                        (ustensil) => ustensil.toLowerCase() === utensil,
                    ),
                ),
            )
            break
    ...
    uniqueItems = [
        ...new Set(
            recipesMatchingData.filter(
                (item) => !activeFilters.includes(item.toLowerCase()),
            ),
        ),
    ]
    ...
    listContainer.innerHTML = uniqueItems
        .map((item) => {
...
function cleanSearchInput(filterType) {
    const inputElement = document.querySelector(
        `input[data-filter-type="${filterType}"]`,
    )
    if (inputElement) {
        inputElement.value = ""
    }
...
function deleteSelectedFilter(event) {
    if (
        ...
        event.target.classList.contains("selected_filter_btn")
    ) {
    ...
    filteredRecipes = recipes.filter((recipe) =>
        filters.every(
            (filter) =>
                ...
                recipe.ustensils.some(
                    (ustensil) => ustensil.toLowerCase() === filter,
                ),
        ),
    )
}
